...
const ClienteCartaDigitalView = () => {
  ...
  useEffect(() => {
    const cargarCarta = async () => {
      setCargando(true);
      try {
        const categoriasData = await getCategoriasByBar(barId);
        setCategorias(categoriasData);
        const productosData = await getProductosByBar(barId);
  ...
  const productosFiltrados = productos.filter(producto => {
    const coincideCategoria = categoriaSeleccionada === 'all' || producto.categoria?.id === parseInt(categoriaSeleccionada);
