...
const EmpleadoPedidosView = () => {
  ...
  const [filtroMesa, setFiltroMesa] = useState('todas'); // NUEVO estado para el filtro de mesa
  ...
  const pedidosAbiertos = pedidos.filter(pedido =>
    (
      pedido.items.some(item => item.estado !== 'entregado') // Alguna línea no entregada
      || !pedido.pagado // No está pagado
    )
  );

  const pedidosCerrados = pedidos.filter(pedido =>
    pedido.items.every(item => item.estado === 'entregado') // Todas las líneas entregadas
    && pedido.pagado // Está pagado
  );
  ...
  // Obtén todas las mesas únicas de los pedidos
  const mesasUnicas = Array.from(new Set(pedidos.map(p => p.mesa))).sort();

  // Filtrado y ordenación
  const pedidosFiltrados = pedidos
    .filter(pedido =>
      (filtroEstado === 'todos' || pedido.estado === filtroEstado) &&
      (filtroMesa === 'todas' || pedido.mesa === filtroMesa)
    )
    .sort((a, b) => {
    ...
    <div className="empleado-pedidos-view">
      ...
      <div className="pedidos-grid">
        {pedidosFiltrados.map(pedido => {
          // Pedido abierto: alguna línea no entregada o no pagado
          const esAbierto = pedido.items.some(item => item.estado !== 'entregado') || !pedido.pagado;
          // Pedido cerrado: todas las líneas entregadas y pagado
          const esCerrado = pedido.items.every(item => item.estado === 'entregado') && pedido.pagado;

          return (
            <div
              key={pedido.id}
              className={`pedido-card estado-${pedido.estado} ${esAbierto ? 'abierto' : ''} ${esCerrado ? 'cerrado' : ''}`}
            >
              ...
              <div className="pedido-footer" style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
                ...
                {pedido.pagado && pedido.metodoPago && (
                  <span className="pedido-metodo-pago" style={{ fontSize: '0.97em', color: '#888', marginTop: 2, display: 'flex', alignItems: 'center', gap: 4 }}>
                    Método de pago:&nbsp;
                    {pedido.metodoPago === 'tarjeta' && <MdCreditCard title="Tarjeta" />}
                    {pedido.metodoPago === 'efectivo' && <MdAttachMoney title="Efectivo" />}
                    <span>{pedido.metodoPago.charAt(0).toUpperCase() + pedido.metodoPago.slice(1)}</span>
                  </span>
                )}
              </div>
            </div>
